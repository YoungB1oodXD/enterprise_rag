...
- **会话管理** — 多会话隔离，切换知识库自动切换会话列表，历史消息持久化
...
| 文档解析 | pdfplumber（PDF）、python-docx（Word）、openpyxl（Excel）、内置解析器（TXT/MD/CSV/JSON/JSONL），OCR 兜底 |
...
| `/v1/knowledge_base/list` | GET | 知识库列表 | 是 |
| `/v1/knowledge_base/{id}/documents` | GET | 文档列表 | 是 |
| `/v1/conversation/list` | GET | 会话列表（指定知识库） | 是 |
